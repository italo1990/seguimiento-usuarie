...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.fichasDeSeguimiento.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.fichas-de-seguimientos.update", [$fichasDeSeguimiento->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('usuarie') ? 'is-invalid' : '' }}" name="usuarie_id" id="usuarie_id">
                    @foreach($usuaries as $id => $entry)
                        <option value="{{ $id }}" {{ (old('usuarie_id') ? old('usuarie_id') : $fichasDeSeguimiento->usuarie->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('profesional') ? 'is-invalid' : '' }}" name="profesional_id" id="profesional_id">
                    @foreach($profesionals as $id => $entry)
                        <option value="{{ $id }}" {{ (old('profesional_id') ? old('profesional_id') : $fichasDeSeguimiento->profesional->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('fecha_y_hora') ? 'is-invalid' : '' }}" type="text" name="fecha_y_hora" id="fecha_y_hora" value="{{ old('fecha_y_hora', $fichasDeSeguimiento->fecha_y_hora) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('comentarios_seguimiento') ? 'is-invalid' : '' }}" name="comentarios_seguimiento" id="comentarios_seguimiento">{!! old('comentarios_seguimiento', $fichasDeSeguimiento->comentarios_seguimiento) !!}</textarea>
